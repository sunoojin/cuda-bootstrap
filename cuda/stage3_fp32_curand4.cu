// Stage 3 — fp32 + curand4 vectorized RNG.
//
// Two optimizations on top of Stage 2:
//   1. fp32 returns array + per-thread accumulators (cache + arithmetic throughput)
//   2. curand4(&state) batched RNG — 1 round produces 4 uints, processes 2 iters
//      (vs Stage 2: 2 rounds for 1 iter, 5 of 8 uints wasted)
//
// Stage 2와 동일한 algorithmic structure (1 perm/block, 256 threads, warp shuffle reduction).
// 최종 reduction은 double로 cast하여 catastrophic cancellation 회피.
//
// Build: nvcc -O3 -arch=sm_86 --use_fast_math -lineinfo stage3_fp32_curand4.cu -o stage3_fp32_curand4
...
int main(int argc, char** argv) {
    ...
    // warmup
    bootstrap_block_fp32<<<B, BLOCK_THREADS>>>(d_ret, N, d_sharpes, p_threshold, seed);
    ...
    printf("Stage3 fp32+curand4: N=%d B=%d threads=%d elapsed=%.3fms throughput=%.3f Mperms/s null_mean=%.6f\n",
